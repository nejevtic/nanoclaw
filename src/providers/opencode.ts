...
registerProviderContainerConfig('opencode', (ctx) => {
  ...
  //
  // Precedence per key: real exported variable > the group's own overrides
  // (container_configs.env) > the global `.env`. The group override lets
  // different agent groups use different providers/models/endpoints (e.g. one
  // group on opencode-go, another on a local Ollama) despite a single host.
  const dotenv = readEnvFile([...PASSTHROUGH_KEYS]);
  let groupEnv: Record<string, unknown> = {};
  try {
    const row = getContainerConfig(ctx.agentGroupId);
    if (row?.env) {
      const parsed: unknown = JSON.parse(row.env);
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        groupEnv = parsed as Record<string, unknown>;
      }
    }
  } catch {
    groupEnv = {};
  ...
  for (const key of PASSTHROUGH_KEYS) {
    const groupValue = groupEnv[key];
    const value =
      ctx.hostEnv[key] ?? (typeof groupValue === 'string' && groupValue ? groupValue : undefined) ?? dotenv[key];
