    ...
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            ...
            // OpenAI-compatible endpoint (Ollama, vLLM, llama.cpp, …).
            //
            // Deliberately NOT pinning `npm: '@ai-sdk/openai-compatible'`
            // (the Chat-Completions transport) even though an older revision did:
            // OpenCode's `openai` provider branch unconditionally calls
            // `provider.responses(model)` (the Responses API) — verified in both
            // the 1.4.17 binary and the latest 1.18.18. The `openai-compatible`
            // package exposes `languageModel`/`chatModel` but NO `.responses`,
            // so loading it makes that call throw
            // `Z.responses is not a function`. The stock OpenAI provider
            // (Responses API) is the working transport for any OpenAI-compatible
            // backend that implements it — Ollama's `/v1/responses` is verified
            // to return completions, and the model/baseURL below drive it.
            // Scoped to `openai` only: other providers (e.g. `openrouter`,
            // set alongside ANTHROPIC_BASE_URL per the documented OpenRouter
            // config) ship their own native ai-sdk package and keep OpenCode's
