...
describe('buildOpenCodeConfig provider transport', () => {
  ...
  it('custom base URL keeps the stock (Responses-API) transport — no chat-completions pin', () => {
    ...
    // OpenCode's `openai` provider branch calls provider.responses(model), so
    // pinning `@ai-sdk/openai-compatible` (which has no `.responses`) throws
    // `Z.responses is not a function`. A custom base URL must drive the stock
    // OpenAI provider instead — hence NO `npm` pin.
  ...
  it('openai with a base URL does NOT pin the Chat Completions transport (regression for Z.responses)', () => {
    process.env.OPENCODE_PROVIDER = 'openai';
    process.env.OPENCODE_MODEL = 'openai/some/local-model';
    process.env.ANTHROPIC_BASE_URL = 'https://inference.example.test/v1';
    const config = buildOpenCodeConfig({});
    const entry = (config.provider as Record<string, Record<string, unknown>>).openai;
    // Regression guard: pinning the chat-completions transport broke OpenCode's
    // `openai` branch (`Z.responses is not a function`). Keep it un-pinned.
    expect(entry.npm).toBeUndefined();
  });
});
